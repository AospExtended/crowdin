...
def push_as_commit(config_files, base_path, path, name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push(f'ssh://{username}@gerrit.aospextended.com:29418/AospExtended/{name}',
...
def submit_gerrit(branch, username, owner):
    ...
    cmd = ['ssh', '-p', '29418',
        f'{username}@gerrit.aospextended.com',
        'gerrit', 'query',
        'status:open',
        f'branch:{branch}',
        ownerArg,
...
def main():
    ...
    xml_main = load_xml(x=f'{base_path}/manifest/snippets/aex.xml')
    if xml_main is None:
        sys.exit(1)
